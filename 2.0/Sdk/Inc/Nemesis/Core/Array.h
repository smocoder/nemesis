...
namespace Nemesis
{
	...
	inline int Array_LinearFind( const Array<T>& a, const U& v )
	{
	...
	inline int Array_LinearFind( const Array<T>& a, const U& v, const Predicate& predicate )
	{
		for ( int i = 0; i < a.Count; ++i )
		{
			if (predicate(a[i], v))
	...
	// This overload uses comparison operators for comparison.
	template <typename T, typename U>
	inline int Array_BinaryFind( const Array<T>& a, const U& v )
	{
		int pivot;
		int start = 0;
		int end = a.Count-1;
	...
	template <typename T, typename U, typename Predicate>
	inline int Array_BinaryFind( const Array<T>& a, const U& v, const Predicate& predicate )
	{
		int cmp;
		int pivot;
		int start = 0;
